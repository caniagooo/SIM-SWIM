...
<?php $component->withAttributes([]); ?>
    <div class="container py-3">
                ...
                <div>
                    ...
                    <div class="d-flex flex-wrap gap-2 small mb-1">
                        ...
                        <span class="badge badge-light-secondary fw-semibold">
                            <i class="bi bi-geo-alt me-1"></i> <?php echo e($course->venue->name ?? 'N/A'); ?>

                        </span>
                        <span class="badge badge-light-success fw-semibold">
                            <i class="bi bi-cash-stack me-1"></i> Rp <?php echo e(number_format($course->price)); ?>

                        </span>
                        <span class="badge <?php echo e((isset($course->payment) && (strtolower($course->payment->status) === 'lunas' || strtolower($course->payment->status) === 'paid')) ? 'badge-light-success' : 'badge-light-danger'); ?> fw-semibold">
                            <?php echo e($course->payment->status ?? 'unpaid'); ?>

                        </span>
                    </div>
                    <?php if($course->description): ?>
                        <div class="text-muted fs-8 mt-1"><?php echo e($course->description); ?></div>
                    <?php endif; ?>
                </div>
                <div class="d-flex gap-2">
                    <a href="<?php echo e(route('courses.index')); ?>" class="btn btn-sm btn-light btn-active-light-primary" title="Kembali ke daftar kursus">
                        <i class="bi bi-arrow-left"></i>
                    </a>
                    <a href="<?php echo e(route('courses.edit', $course->id)); ?>" class="btn btn-sm btn-light-primary" title="Edit kursus">
            ...
            <div class="col-6 col-md-3">
                <div class="card card-flush text-center border-0 shadow-sm h-100">
                    <div class="card-body py-3 px-2">
                        ...
                        <div class="fw-bold fs-6"><?php echo e($course->start_date ? $course->start_date->translatedFormat('d M Y') : 'N/A'); ?></div>
                    </div>
                </div>
            </div>
            <div class="col-6 col-md-3">
                <div class="card card-flush text-center border-0 shadow-sm h-100">
                    <div class="card-body py-3 px-2">
                        ...
                        <div class="fw-bold fs-6"><?php echo e($course->valid_until ? $course->valid_until->translatedFormat('d M Y') : 'N/A'); ?></div>
                    </div>
                </div>
            </div>
            <div class="col-6 col-md-3">
                <div class="card card-flush text-center border-0 shadow-sm h-100">
                    <div class="card-body py-3 px-2">
                        ...
                        <div class="fw-bold fs-6">
                            <?php echo e($course->sessions->where('status', 'completed')->count()); ?> / <?php echo e($course->max_sessions ?? 'N/A'); ?>
        ...
        <div class="tab-content" id="courseTabsContent">
                                    ...
                                    <?php $__empty_1 = true; $__currentLoopData = $course->students; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $student): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <img src="<?php echo e(optional($student->user)->profile_photo_path ?? asset('assets/media/avatars/default-avatar.png')); ?>" alt="Avatar" class="symbol symbol-30px symbol-circle">
                                            </td>
                                            <td>
                                                <a href="<?php echo e(route('students.show', $student->id)); ?>" class="fw-semibold text-primary text-hover-dark fs-8 text-decoration-none">
                                                    <?php echo e(optional($student->user)->name ?? '-'); ?>

                                                </a>
                                            </td>
                                            <td class="text-center">
                                                <span class="badge badge-light-info fw-semibold"><?php echo e($attendanceCount); ?>/<?php echo e($maxSessions); ?></span>
                                            </td>
                                            <td class="text-center">
                                                <span class="badge badge-light-success fw-semibold"><?php echo e($averageScore); ?></span>
                                            </td>
                                            <td class="text-center">
                                                <button class="btn btn-icon btn-sm btn-light-primary" data-bs-toggle="modal" data-bs-target="#gradeModal<?php echo e($student->id); ?>">
                            ...
                            <table class="table table-row-dashed table-row-gray-200 align-middle gy-2 mb-0">
                                ...
                                <tbody>
                                    <?php $__currentLoopData = $course->sessions->whereIn('status', ['scheduled', 'completed','rescheduled','canceled']); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $session): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <tr>
                                            <td class="text-center text-gray-500 fs-8"><?php echo e($loop->iteration); ?></td>
                                            <td class="text-center fs-8"><?php echo e(\Carbon\Carbon::parse($session->session_date)->format('d M Y')); ?></td>
                                            <td class="text-center fs-8"><?php echo e(\Carbon\Carbon::parse($session->start_time)->format('H:i')); ?></td>
                                            <td class="text-center fs-8"><?php echo e(\Carbon\Carbon::parse($session->end_time)->format('H:i')); ?></td>
                                            <td class="text-center">
                                                <span class="badge
                                                    <?php if($session->status === 'scheduled'): ?> badge-light-info
                                                    <?php elseif($session->status === 'rescheduled'): ?> badge-light-warning
                                                    <?php elseif($session->status === 'canceled'): ?> badge-light-danger
                                            ...
                                            <td>
                                                <div class="d-flex justify-content-end gap-1">
                                                    <button class="btn btn-icon btn-sm btn-light-success btnAttendance" data-session-id="<?php echo e($session->id); ?>" title="Kehadiran">
                                                    ...
                                                    <?php endif; ?>
                                                    <button class="btn btn-icon btn-sm btn-light-danger btnDeleteSession" data-session-id="<?php echo e($session->id); ?>" title="Hapus">
            ...
            <!-- Materi Tab -->
            <div class="tab-pane fade <?php echo e($activeTab === 'materials' ? 'show active' : ''); ?>" id="materials" role="tabpanel" aria-labelledby="materials-tab">
                <div class="card card-flush border-0 shadow-sm mb-3">
                    <div class="card-body p-0">
                        ...
                        <div class="row g-0">
                                    ...
                                    <ul>
                                        <?php $__currentLoopData = $course->materials; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $material): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <li class="list-group-item px-0 py-3"></li>
                                                <div class="fw-semibold mb-1"><?php echo e($material->name); ?></div>
                                                <div class="d-flex flex-wrap gap-2 mb-3" style="border-bottom: 1px dashed #e4e6ef; padding-bottom: 0.5rem;">
                                                    <span class="badge badge-light-info fw-semibold">
                                                        Estimasi Sesi: <?php echo e($material->estimated_sessions ?? '-'); ?>

                                                    </span>
                                                    <span class="badge badge-light-warning fw-semibold">
                                                        Min. Skor: <?php echo e($material->minimum_score ?? '-'); ?>

                                                    </span>
                                                    <span class="badge badge-light-primary fw-semibold">
                                                        Kategori: <?php echo e($material->level ?? '-'); ?>
                            ...
                            <div class="col-md-5 px-4 py-3">
                                <div class="mb-4">
                                    ...
                                    <?php if(!empty($course->basic_skills)): ?>
                                        <ul class="mb-3 ps-3 fs-8" style="list-style: disc;">
                                            <?php $__currentLoopData = is_array($course->basic_skills) ? $course->basic_skills : explode(',', $course->basic_skills); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $skill): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <li class="mb-1 text-gray-800"><?php echo e(trim($skill)); ?></li>
                                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                        </ul>
                                    <?php else: ?>
                                ...
                                <div>
                                    ...
                                    <ul class="mb-0 ps-3 fs-8" style="list-style: circle;">
                                        <li class="mb-1">
                                            ...
                                            <span class="fw-bold text-primary"><?php echo e($totalEstimatedSessions ?: '-'); ?></span>
                                        </li>
                                        <li>
                                            <span class="text-gray-600">Rata-rata Minimum Skor:</span>
                                            <span class="fw-bold text-success"><?php echo e($averageMinScore); ?></span>
                                        </li>
                                    </ul>
                                </div>
                            </div>
...
<?php endif; ?>
<?php /**PATH C:\Users\JITU\swim\resources\views/courses/show.blade.php ENDPATH**/ ?>